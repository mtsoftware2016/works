...
class OrderRequest(http.Controller):
    ...
    # GET Vendors
    @http.route('/get_vendors_list', type='json', auth='public', website=True, csrf=False)
    ...
    @http.route(['/order_requests', '/order_requests/page/<int:page>'], type='http', auth="user", website=True)
    def display_order_requests(self, page=1, date_begin=None, date_end=None, sortby=None, search=None,
                               search_in='name', **kw):
        authorization = False
        values = {}
        ...
        order_requests = request.env['order.request'].search(domain, limit=item_per_page, offset=pager['offset'],
                                                             order=order)
    ...
    def render_to_order_request_create(self, **kw):
        ...
        request.env.cr.execute("SELECT * FROM  product_product_res_users_rel where res_users_id = %s",
                               (request.uid,))
        ...
        product_filtered = products.filtered(lambda product: product.categ_id.is_employee_autorised)
        vals = {
    ...
    def save_order_request(self, **kw):
        ...
        tax = [employee.tax_id.id] if employee.tax_id else product.sudo().supplier_taxes_id.ids
        lines.append((0, 0, {
        ...
        list_attchement_ids = []
        upload_file_count = int(kw.get('upload_file_count')) if kw.get('upload_file_count') else 0
        ...
        order_request = request.env['order.request'].create({
            ...
            'line_ids': lines,
            'user_id': request.uid,
            'attachment_ids': [(6, 0, list_attachment_stored)] if list_attachment_stored else None
        })
        current_user.sudo().write({
            'authorization_create': False
        })
        # Send Email To Manager
        user_manager = request.env['res.users'].sudo().search([('is_manager', '=', True)], limit=1)
        ...
        request.env.context = context
        template_id.sudo().send_mail(order_request.id, force_send=True, raise_exception=True)

        return http.request.redirect('/order_requests')

    @http.route('/order/<model("order.request"):order>', type='http', auth="public", website=True)
