...
class ResUsers(models.Model):
    ...
    authorization_create = fields.Boolean(
        string="Authorization For Creation Purchase", default=True
    )
    ...
    def create(self, vals):
        ...
        if portal != portal_id and (vals.get("is_user") or vals.get("is_manager")):
            raise ValidationError(
                "Sorry you are not allowed please Change to portal User"
            )
        return res
    ...
    def write(self, vals):
        ...
        if (
            portal
            and portal != portal_id
            and (
                vals.get("is_user")
                or vals.get("is_manager")
                or self.is_user
                or self.is_manager
            )
        ):
