...
class Product(models.Model):
    ...
    buybox_seller_name = fields.Char(
        string='Buybox seller name'
    )

    buybox_seller_price = fields.Monetary(
        string='Buybox seller price'
    )

    first_seller_name = fields.Text(
        string='First seller name'
    )
    first_seller_price = fields.Monetary(
        string='First seller price'
    )

    preconisation_odoo = fields.Monetary(
        string='Préconisation Odoo'
    )

    price_plafond = fields.Monetary(
        string='Prix plafond'
    )

    price_plancher = fields.Monetary(
        string='Prix plancher',
        compute='_compute_price_plancher'
    )
    ...
    figer = fields.Boolean(
        string='Figer le repricing'
    )

    restauring = fields.Boolean(
        string='Récupération Buybox'
    )
    ...
    status_repricing = fields.Selection([
        ...
        ('buybox_with_concurrent_plafond', 'Buybox avec concurrent prix en plafond'),
    ...
    buybox_seller_name_old = fields.Char(
        string='Ancien Buybox seller name'
    )

    buybox_seller_price_old = fields.Monetary(
        string='Ancien Buybox seller price'
    )

    first_seller_name_old = fields.Text(
        string='Ancien First seller name'
    )
    first_seller_price_old = fields.Monetary(
        string='Ancien First seller price'
    )

    preconisation_odoo_old = fields.Monetary(
        string='Ancien Préconisation Odoo'
    )
    ...
    figer_old = fields.Boolean(
        string='Ancien Figer le repricing'
    )

    restauring_old = fields.Boolean(
        string='Ancien Récupération Buybox'
    )
    ...
    product_not_found = fields.Char(
        string='Input'
    )

    seller_outtilage_online = fields.Float(
        string='Seller Outillage Online'
    )

    buybox_shipping = fields.Monetary(
        string='Buybox Shipping'
    )
    buybox_stock = fields.Integer(
        string='Buybox stock'
    )

    buybox_mf = fields.Text(
        string='Buybox MF'
    )

    order_number_oo_mf = fields.Integer(
        string='Nombre de commandes OO+MF',
        compute='_compute_order_number_oo_mf'
    )
    is_price_plafond_off = fields.Boolean(
        string='Désactiver prix plafond'
    )
    ...
    is_cron_price_plancher_synchronized = fields.Boolean(
        string='Is Cron Synchronized',
        default=False
    )

    is_batch_1 = fields.Boolean(
        string='Batch 1',
        default=False
    )

    is_batch_2 = fields.Boolean(
        string='Batch 2',
        default=False
    )

    is_batch_3 = fields.Boolean(
        string='Batch 3',
        default=False
    )

    date_scrapping = fields.Date(
        string='Date Scrapping'
    )
    ...
    def _compute_price_plancher(self):
        for rec in self:
            part_1 = rec.x_studio_prix_dachat + rec.price_logistique + rec.price_transport + rec.fee_approche
    ...
    def _reset_pricing_for_products(self):
        products = self.env['product.product'].search([('product_not_found', '!=', False)])
    ...
    def _update_pricing_products(self):
        ...
        for product in products:
            ...
            if not product.status_repricing \
                ...
                product.product_tmpl_id.lst_price = product.preconisation_odoo if product.preconisation_odoo > 0 else product.lst_price

            # prix parfait 2/3, buybox récupérée
            if not product.status_repricing and product.restauring_old \
            ...
            if not product.status_repricing and product.figer_old \
            ...
            if not product.status_repricing \
                ...
                result_price = product.price_plafond if product.lst_price * 1.005 >= product.price_plafond else product.lst_price * 1.005
                product.preconisation_odoo = result_price
                product.status_repricing = 'buybox_with_concurrent' if round(product.preconisation_odoo, 2) < round(
                    product.price_plafond, 2) else 'buybox_with_concurrent_plafond'
                product.scrapping_status = 'all_conforme'
                product.product_tmpl_id.lst_price = product.preconisation_odoo if product.preconisation_odoo > 0 else product.lst_price
            ...
            if not product.status_repricing \
                ...
                result_price = product.price_plancher if product.lst_price * 0.995 <= product.price_plancher else product.lst_price * 0.995
                product.preconisation_odoo = result_price
                product.status_repricing = 'no_buybox_price_down' if round(product.preconisation_odoo, 2) > round(
                    product.price_plancher, 2) else 'no_buybox_price_plancher'
                product.scrapping_status = 'all_conforme'
                product.product_tmpl_id.lst_price = product.preconisation_odoo if product.preconisation_odoo > 0 else product.lst_price

            # Pas de Buybox MF
            if not product.status_repricing and product.buybox_seller_name not in OUTILLAGE_ONLINE \
                ...
                result_price = price_purchase if price_sale <= price_purchase else price_sale
                ...
                result_price_preconisation = price_sale if price_sale >= price_purchase else price_purchase
                product.preconisation_odoo = result_price_preconisation
                product.product_tmpl_id.lst_price = product.preconisation_odoo if product.preconisation_odoo > 0 else product.lst_price
                product.status_repricing = 'no_buybox_mf_price_lost' if round(product.preconisation_odoo, 2) <= round(
                    price_purchase, 2) else 'no_buybox_mf'
    ...
    def _update_products_price_plafond_plancher(self):
        ...
        next_call_date = (fields.Datetime.now() + datetime.timedelta(days=1)).strftime('%Y-%m-%d 00:00:00')
        numbercall = 7
        # RESET Synchronization Products
        if len(all_products) == products_synchronized:
            ...
            self._cr.execute("UPDATE ir_cron SET nextcall=%s, numbercall=%s WHERE id=%s",
                             (next_call_date, numbercall, CRON.id))

        for product in products:
            product.is_cron_price_plancher_synchronized = True

            if product.status_repricing_old in ['no_buybox_mf', 'no_buybox_mf_price_lost']:
                continue
    ...
    def get_price_plafond(self, coeff):
        result = 0
        if coeff:
            price_1 = (self.x_studio_prix_dachat + self.price_logistique + self.price_transport + self.fee_approche) - (
        ...
        if not self.is_price_plafond_off:
            ...
            for route in self.route_ids:
                ...
                if route.name == 'Acheter':
                    order_point = self.orderpoint_ids[0] if self.orderpoint_ids else False
                    available_days = 0
                    if order_point:
                        available_days = max(order_point.number_of_availability_days,
                    ...
                    coeff = celing_price.price_list_route_buy.filtered(
                        lambda c: c.order_number == self.order_number_oo_mf
                                  and c.number_of_availability_days <= available_days <= c.number_of_availability_days_2
                    ).multiplier
