...
class BrightDataCollector(models.Model):
    ...
    collector = fields.Char(string="Collecteur", required=True)
    bearer_token = fields.Char(string="Bearer Token", required=True)
    is_activated = fields.Boolean(string="Active")
        ...
        for i in range(0, int(file_count)):
            ...
            if file_size > 0:
                ...
                for index, row in data.iterrows():
                    ...
                    product_id = (
                        int(column_id) if self.can_convert_to_int(column_id) else False
                    )
                    ...
                    buybox_seller_price = (
                        product.buybox_seller_price
                        if pd.isnull(column_buybox_seller_price)
                        else float(column_buybox_seller_price)
                    )
                    buybox_seller_name = (
                        product.buybox_seller_name
                        if pd.isnull(column_buybox_seller_name)
                        else column_buybox_seller_name
                    )
                    first_seller_name = (
                        product.first_seller_name
                        if pd.isnull(column_first_seller_name)
                        else column_first_seller_name
                    )
                    first_seller_price = (
                        product.first_seller_price
                        if pd.isnull(column_first_seller_price)
                        else float(column_first_seller_price)
                    )
                    rating_mano = (
                        product.x_studio_ratings_mano_1
                        if pd.isnull(column_products_review)
                        else int(column_products_review)
                    )
                    buybox_shipping = (
                        product.x_studio_ratings_mano_1
                        if pd.isnull(column_buybox_shipping)
                        else int(column_buybox_shipping)
                    )
                    buybox_stock = (
                        product.x_studio_ratings_mano_1
                        if pd.isnull(column_buybox_stock)
                        else int(column_buybox_stock)
                    )
                    buybox_mf = (
                        product.buybox_mf
                        if pd.isnull(column_buybox_mf)
                        else column_buybox_mf
                    )
                    date_value = False
                    if column_date_scrapping:
                        date_scrapping_excel = column_date_scrapping.replace("/", "-")
                        date_value = datetime.strptime(
                            date_scrapping_excel, "%d-%m-%Y"
                        ).date()
                    date_scrapping = (
                        product.date_scrapping if not date_value else date_value
                    )
                    ...
                    product.write(
                        {
                            ...
                            "seller_outtilage_online": float(
                                column_seller_outillage_online
                            ),
                        }
                    )
                shutil.move(
                    xml_file,
                    "/home/odoo/old_scrapping/scrapping" + "_" + date_now + ".xlsx",
                )
            break
    ...
    def collect_data_products(self):
        ...
        fields = ["id", "default_code", "lst_price", "x_studio_url_mano"]
        ...
        data = []
        collectors = self.env["bright.data.collector"].search(
            [("is_activated", "=", True)]
        )
        # Clean repository
        self._clean_repository_bright_data_from_server()
