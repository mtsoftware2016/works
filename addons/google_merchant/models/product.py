...
class ProductProduct(models.Model):
    ...
    def _check_availability_stock(self, product):
        ...
        stock = 'out_of_stock'
        if (product.virtual_available_without_incoming_qty_main >= 1
    ...
    def _prepare_custom_label(self, product):
        ...
        """
        custom_label = ''
        if product.order_count_12 >= 20 and product.status_repricing_old in BUYBOX_STATUS and (
                product.virtual_available_without_incoming_qty_main >= 1
                or product.stock_virtual_drop >= 1 or product.announced_sale_delay <= 5):
            custom_label = 'Ventes &gt;=20, Stock et Buybox '

        elif (
                19 >= product.order_count_12 >= 5
                and product.status_repricing_old in BUYBOX_STATUS
                and (product.virtual_available_without_incoming_qty_main >= 1
                     or product.stock_virtual_drop >= 1 or product.announced_sale_delay <= 5)
        ):
    ...
    def _prepare_additional_image_link(self, root, item, product):
        ...
        if product.x_studio_url_image2:
            additional_image_link_1.appendChild(root.createTextNode(product.x_studio_url_image2))
            item.appendChild(additional_image_link_1)

        if product.x_studio_url_image3_1:
            additional_image_link_2.appendChild(root.createTextNode(product.x_studio_url_image3_1))
            item.appendChild(additional_image_link_2)

        if product.x_studio_url_image4:
            additional_image_link_3.appendChild(root.createTextNode(product.x_studio_url_image4))
            item.appendChild(additional_image_link_3)

        if product.x_studio_url_image5:
            additional_image_link_4.appendChild(root.createTextNode(product.x_studio_url_image5))
            item.appendChild(additional_image_link_4)

        if product.x_studio_url_image6:
            additional_image_link_5.appendChild(root.createTextNode(product.x_studio_url_image6))
    ...
    def _create_google_merchant_product_file(self):
        ...
        description.appendChild(root.createTextNode('Kobleo by Outillage Online: bricoler, rénover, bâtir'))
        ...
        for product in products:
            ...
            id.appendChild(root.createTextNode(
                str(product.x_studio_odoo_id) if product.x_studio_odoo_id else '')
            )
            title.appendChild(root.createTextNode(product.long_name if product.long_name else ''))
            description.appendChild(root.createTextNode(
                self.text_from_html(product.website_description) if product.website_description else '')
            )
            ...
            availability.appendChild(root.createTextNode(self._check_availability_stock(product)))
            lst_price = f'{str(round(product.sale_price_kobleo, 2))} {product.currency_id.name}' if product.sale_price_kobleo else ''
            price_cost_of_goods_sold = f'{str(round(product.price, 2))} {product.currency_id.name}' if product.price else ''
            price.appendChild(root.createTextNode(lst_price))
            condition.appendChild(root.createTextNode(self._check_product_condition(product)))
            custom_label_0.appendChild(root.createTextNode(self._prepare_custom_label(product)))
            if product.benefice_net >= 15:
                custom_label_1.appendChild(root.createTextNode('Mage nette >= 15€ '))
            brand.appendChild(root.createTextNode(product.product_brand_id.name if product.product_brand_id else ''))
            ...
            gtin.appendChild(root.createTextNode(product.x_studio_code_barre if product.x_studio_code_barre else ''))
            ships_from_country.appendChild(root.createTextNode('FR'))
            cost_of_goods_sold.appendChild(root.createTextNode(price_cost_of_goods_sold))
            product_length.appendChild(root.createTextNode(str(product.length) + ' cm' if product.length else ''))
            product_width.appendChild(root.createTextNode(str(product.width) + ' cm' if product.width else ''))
            product_height.appendChild(root.createTextNode(str(product.height) + ' cm' if product.height else ''))
            product_weight.appendChild(root.createTextNode(str(product.weight) + ' kg' if product.weight else ''))
            item.appendChild(id)
