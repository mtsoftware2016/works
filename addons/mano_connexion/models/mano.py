...
class ManoMano(models.Model):
    ...
    def get_default_payment_method(self, journal_id):
        ...
        payment_method_ids = self.env['account.journal'].browse(journal_id)._default_inbound_payment_methods()
    ...
    def test_connexion_mano(self):
        request = requests.get(
            'https://ws.monechelle.com/?login=' + self.login + '&password=' + self.password + '&method=get_orders')
        text = request.text.replace('<![CDATA[', '').replace(']]>', '')
        if request.status_code == 200 and "Incorrect" not in text:
            raise UserError(
                _("Connexion avec Mano sucess"))
        else:
            raise UserError(
    ...
    def get_orders_mf(self, mano_connexion):
        ...
        url = 'https://ws.monechelle.com/?login=' + mano_connexion.login + '&password=' \
              + mano_connexion.password + '&method=get_last_orders' + '&date=' + str(date_arg)
        r = requests.get(url)
    ...
    def get_orders(self, mano_connexion):
        """ @params mano_connexion: credentials connexion to mano
            @return: Orders Mano method (list)"""
        url = 'https://ws.monechelle.com/?login=' + mano_connexion.login + '&password=' + mano_connexion.password + '&method=get_orders'
        r = requests.get(url)
        data = et.fromstring(r.text.encode("utf-8"))
        return data.xpath('//order')
    ...
    def get_last_orders(self, mano_connexion, date):
        """ @params mano_connexion: credentials connexion to mano
            @return: Orders Mano method (list)"""
        url = 'https://ws.monechelle.com/?login=' + mano_connexion.login + '&password=' + mano_connexion.password + '&method=get_last_orders' + '&date=' + date
        r = requests.get(url)
        data = et.fromstring(r.text.encode("utf-8"))
        return data.xpath('//order')
    ...
    def accept_order(self, order_ref, mano_connexion):
        ...
        url = 'https://ws.monechelle.com/?login=' + mano_connexion.login + '&password=' + mano_connexion.password + '&method=accept_order&order_ref=' + order_ref
        requests.get(url)
    ...
    def refuse_order(self, order_ref, mano_connexion):
        ...
        url = 'https://ws.monechelle.com/?login=' + mano_connexion.login + '&password=' + mano_connexion.password + '&method=refuse_order&order_ref=' + order_ref
        requests.get(url)
    ...
    def create_order_from_mano_mf(self):
        ...
        for i, order in enumerate(orders):
            if not self.env['sale.order'].search([('x_studio_rfrence_marketplace', '=', order.find('order_ref').text)]):
    ...
    def create_shipping(self, order, mano_connexion):
        ...
        if picking and not picking.shiping_mano_active and picking.carrier_tracking_ref and picking.carrier_id.tracking_url:
            tracking_url_carrier = picking.carrier_id.tracking_url.replace('@', picking.carrier_tracking_ref)
            url = 'https://ws.monechelle.com/?login=' + mano_connexion.login + '&password=' + mano_connexion.password + '&method=create_shipping&order_ref=' \
                  + order.x_studio_rfrence_marketplace + '&tracking_number=' + picking.carrier_tracking_ref + '&carrier=' + \
                  picking.carrier_id.name + '&tracking_url=' + tracking_url_carrier
            requests.get(url)
    ...
    def create_sale_order(self, order, country):
        ...
        template_id_creation = self.env.ref('mano_connexion.mannon_error_email_creation')
        # Create partner shipping address
        partners = []
        country_by_name_shipping = self.env['res.country'].search(
            [('name', 'like', order.find('shipping_address/country').text)]).id
        country_by_code_shipping = self.env['res.country'].search(
            [('code', 'like', order.find('shipping_address/country_iso').text)]).id
        ...
        partners.append((0, 0, {
            ...
            'name': order.find('shipping_address/firstname').text + ' ' + order.find('shipping_address/lastname').text,
            ...
            'country_id': country_by_name_shipping if country_by_name_shipping else country_by_code_shipping,
            'company_mano': order.find('shipping_address/company').text,
        }))
        # Create partner billing address
        country_by_name = self.env['res.country'].search(
            [('name', 'like', order.find('billing_address/country').text)]).id
        country_by_code = self.env['res.country'].search(
            [('code', 'like', order.find('billing_address/country_iso').text)]).id

        partners.append((0, 0, {
            'type': 'invoice',
            'is_professional': order.find('billing_address/is_professional').text,
            ...
            'name': order.find('billing_address/firstname').text + ' ' + order.find('billing_address/lastname').text,
            ...
            'country_id': country_by_name if country_by_name else country_by_code,
            'company_mano': order.find('billing_address/company').text,
        }))
        partner = self.env['res.partner'].create({
            'name': order.find('firstname').text + ' ' + order.find('lastname').text,
            'email': order.find('billing_address/email').text,
            'customer_rank': 1,
            'child_ids': partners
        })
        ...
        for i in range(1, int(len(order.findall('products/product')) + 1)):
            product = self.env['product.product'].search(
                [('default_code', '=', order.find('products/product[' + str(i) + ']/sku').text)])
            if len(product) >= 2:
            ...
            if not product:
                context = dict(self.env.context)
                context.update({
                    'product_1': order.find('products/product[' + str(i) + ']/sku').text
                })
                self.env.context = context
            ...
            if 'TNT Express' in order.find('products/product[' + str(i) + ']/carrier').text:
                is_tnt_express = True
            lines.append((0, 0, {
                'product_id': self.env['product.product'].search(
                    [('default_code', '=', order.find('products/product[' + str(i) + ']/sku').text)]).id,
                'product_uom_qty': order.find('products/product[' + str(i) + ']/quantity').text,
                'price_unit': float(order.find('products/product[' + str(i) + ']/price').text),
                'name': order.find('products/product[' + str(i) + ']/title').text
            }))
            # SHipping Price
            for route in product.route_ids:
                if route.name == 'Livraison directe':
                    is_dropshiping = True
        if 'is_mf' not in country and not self.check_quantity_products(product.ids) and not is_dropshiping:
            return False

        if float(order.find('shipping_price').text) > 0:
            lines.append((0, 0, {
                'product_id': self.env['product.product'].search(
                    [('default_code', '=', 'SHIP')]).id,
                'product_uom_qty': 1,
        ...
        if not sale_order_rejected:
            vals = {
                ...
                'is_mmf': 'Mano Fulfillment' if order.find('is_mmf').text == '1' else 'Outillage Online Fulfillment',
                ...
                'relay_id': order.find('relay/id').text if order.find('relay/id') is not None else '',
                'relay_name': order.find('relay/name').text if order.find('relay/name') is not None else '',
                'relay_zipcode': order.find('relay/zipcode').text if order.find('relay/zipcode') is not None else '',
                'relay_city': order.find('relay/city').text if order.find('relay/city') is not None else '',
                'relay_country': order.find('relay/country').text if order.find('relay/country') is not None else '',
                'relay_address': order.find('relay/address').text if order.find('relay/address') is not None else ''
            }
            if country == 'is_mf':
                vals.update({
                    'warehouse_id': 2
            ...
            context = {}
            if invoice:
                ctx = {'default_invoice_ids': [[4, invoice.id, None]], 'active_model': 'account.move',
                       'journal_type': 'sale',
                       'search_disable_custom_filters': True, 'active_ids': [invoice.id], 'type': 'out_invoice',
                       'active_id': invoice.id}
                context.update(ctx)
                fields = ['communication', 'currency_id', 'invoice_ids', 'payment_difference', 'partner_id',
                          'payment_method_id', 'payment_difference_handling',
                          'journal_id', 'state', 'writeoff_account_id', 'payment_date', 'partner_type',
                          'hide_payment_method', 'payment_method_code', 'amount', 'payment_type']
    ...
    def create_order_from_mano(self):
        ...
        for order_germany in orders_germany:
            if not self.env['sale.order'].search(
                    [('x_studio_rfrence_marketplace', '=', order_germany.find('order_ref').text)]):
                sale_order = self.create_sale_order(order_germany, 'Germany')
                if sale_order:
                    self.accept_order(order_germany.find('order_ref').text, mano_connexion_germany)
                else:
                    self.refuse_order(order_germany.find('order_ref').text, mano_connexion_germany)

        for order_spain in orders_spain:
            if not self.env['sale.order'].search(
                    [('x_studio_rfrence_marketplace', '=', order_spain.find('order_ref').text)]):
                sale_order = self.create_sale_order(order_spain, 'Spain')
                if sale_order:
                    self.accept_order(order_spain.find('order_ref').text, mano_connexion_spain)
                else:
                    self.refuse_order(order_spain.find('order_ref').text, mano_connexion_spain)

        for order_italy in orders_italy:
            if not self.env['sale.order'].search(
                    [('x_studio_rfrence_marketplace', '=', order_italy.find('order_ref').text)]):
                sale_order = self.create_sale_order(order_italy, 'Italy')
                if sale_order:
                    self.accept_order(order_italy.find('order_ref').text, mano_connexion_italy)
                else:
                    self.refuse_order(order_italy.find('order_ref').text, mano_connexion_italy)

        for order_pro in orders_pro_france:
            if not self.env['sale.order'].search(
                    [('x_studio_rfrence_marketplace', '=', order_pro.find('order_ref').text)]):
                sale_order = self.create_sale_order(order_pro, 'France Pro')
                if sale_order:
                    self.accept_order(order_pro.find('order_ref').text, mano_connexion_france_pro)
                else:
                    self.refuse_order(order_pro.find('order_ref').text, mano_connexion_france_pro)

    @api.model
    def create_shipping_cron(self):
        mano_connexion = self.env.ref('mano_connexion.mano_connexion')
        mano_connexion_france_pro = self.env.ref('mano_connexion.mano_france_pro')
        orders_french_pro = self.env['sale.order'].search([('is_mano', '=', True), ('is_french_pro', '=', True)])
        orders = self.env['sale.order'].search([('is_mano', '=', True), ('is_french', '=', True)])
    ...
    def create_shipping_cron_spain(self):
        mano_connexion_spain = self.env.ref('mano_connexion.mano_connexion_spain')
        orders = self.env['sale.order'].search([('is_mano', '=', True), ('is_spain', '=', True)])
        for order in orders:
    ...
    def create_shipping_cron_italy(self):
        mano_connexion_italy = self.env.ref('mano_connexion.mano_connexion_italie')
        orders = self.env['sale.order'].search([('is_mano', '=', True), ('is_italy', '=', True)])
        for order in orders:
    ...
    def create_shipping_cron_germany(self):
        mano_connexion_germany = self.env.ref('mano_connexion.mano_connexion_germany')
        orders = self.env['sale.order'].search([('is_mano', '=', True), ('is_germany', '=', True)])
        for order in orders:
    ...
    def update_offers_mano(self):
        ...
        for product in products:
            items.append({
                ...
                "price": {
                    "price_vat_included": product.lst_price
                },
                "stock": {
                    "quantity": int(product.virtual_available_without_incoming_qty_main)
                }
            })
        payload = {
            "content": [{
                "seller_contract_id": int(SELLER_CONTRACT_ID),
                "items": items
            }
            ]
        }
        ...
        if RESPONSE_ERRORS:
            ...
            for error in RESPONSE_ERRORS:
                if error.get('api_error_code') not in ['10404', '10405', '10412', '10409']:
    ...
    def update_offers_mano_fr(self):
        ...
        url = 'https://partnersapi.manomano.com/api/v2/offer-information/offers'
        for product in products:
            route = product.route_ids.filtered(lambda route: route.name == 'Livraison directe')
            check_route = 5 if route \
