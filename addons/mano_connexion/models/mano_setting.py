...
class ManoSetting(models.Model):
    ...
    def sale_order_resolver(self):
        ...
        orders = mano_object.get_last_orders(self.mano_connexion, EndDate.strftime('%y-%m-%d'))
        refs = []
        ...
        for order in orders:
            if order.find('order_ref').text in refs:
                if not self.env['sale.order'].search([('x_studio_rfrence_marketplace', '=', order.find('order_ref').text)]):
                    ...
                    if sale_order:
                        mano_object.accept_order(order.find('order_ref').text, self.mano_connexion)
                    else:
                        mano_object.refuse_order(order.find('order_ref').text, self.mano_connexion)
